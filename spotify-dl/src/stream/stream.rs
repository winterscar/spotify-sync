...
impl Stream {
    ...
    pub async fn stream(&self, track: Track) -> Result<StreamEventChannel> {
        ...
        tokio::spawn(async move {
            ...
            match tryhard::retry_fn(|| async { Self::load(player.clone(), &track).await })
                ...
                .await
            {
                Ok(_) => tracing::info!("Track loaded successfully: {:?}", track_uri),
                Err(e) => {
                    tracing::error!("Failed to load track: {:?}, error: {:?}", track_uri, e);
                    Self::send_event(
                        &tx,
            ...
            tracing::info!("Streaming track: {:?}", track_uri);
    ...
    async fn load(player: Arc<Player>, track: &Track) -> Result<()> {
        ...
        tracing::info!("Loading track: {:?}", track_uri);
        loop {
            match player.get_player_event_channel().recv().await {
                ...
                | Some(PlayerEvent::EndOfTrack { .. }) => {
                    tracing::info!("Player started playing track: {:?}", track_uri);
                    break;
                }
                Some(PlayerEvent::Unavailable { .. }) => {
                    tracing::info!("Track is unavailable: {:?}", track_uri);
                    return Err(anyhow::anyhow!("Could not load track: {:?}", track_uri));
                }
                _ => {
                    // Ignore other events
                }
