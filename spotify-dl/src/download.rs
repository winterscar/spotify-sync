...
impl Downloader {
    ...
    async fn download_track(&self, track: Track, options: &DownloadOptions) -> Result<()> {
        ...
        // Extract track ID from URI for filename
        let track_id = match &track.uri {
            librespot::core::spotify_uri::SpotifyUri::Track { id } => id.to_base62()?,
            librespot::core::spotify_uri::SpotifyUri::Episode { id } => id.to_base62()?,
            _ => return Err(anyhow::anyhow!("Invalid track URI")),
        };
